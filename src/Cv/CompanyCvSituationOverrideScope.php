...
/**
 * @brief Whitelist for per-company Situation section override JSON (subset of global CV profile).
 */
final class CompanyCvSituationOverrideScope
{
    /**
     * @var list<string>
     */
    ...
    /**
     * @brief Extract Situation-related keys from a full CV profile payload.
     *
    ...
    public static function sanitizeForPersistence(array $payload): array
    {
        ...
        $map = $sanitized[SituationContentContract::KEY_CONTENT_BY_LOCALE] ?? null;
        if (!is_array($map)) {
            return $sanitized;
        }

        $normalizedMap = [];
        foreach ($map as $locale => $row) {
            ...
            if ($normalized !== null) {
                $normalizedMap[$locale] = $normalized;
            }
        }

        if ($normalizedMap !== []) {
            $sanitized[SituationContentContract::KEY_CONTENT_BY_LOCALE] = $normalizedMap;
        } else {
            unset($sanitized[SituationContentContract::KEY_CONTENT_BY_LOCALE]);
        }
    ...
    /**
     * @brief Merge Situation override keys into a resolved CV payload.
     *
     * @param array<string, mixed> $basePayload Default merged CV payload.
     * @param array<string, mixed> $overridePayload Company Situation override slice.
     * @return array<string, mixed>
     * @date 2026-06-01
     * @author Stephane H.
     */
