...
/**
 * @brief Whitelist for per-company Education section override JSON.
 */
final class CompanyCvEducationOverrideScope
{
    /**
     * @var list<string>
     */
    ...
    /**
     * @brief Extract Education-related keys from a full CV profile payload.
     *
    ...
    public static function sanitizeForPersistence(array $payload): array
    {
        ...
        $map = $sanitized[EducationContract::KEY_ENTRIES_BY_LOCALE] ?? null;
        if (!is_array($map)) {
            return $sanitized;
        }

        $normalizedMap = EducationContract::normalizeEntriesByLocale($map);
        ...
        $sanitized[EducationContract::KEY_ENTRIES_BY_LOCALE] = $normalizedMap;

        return $sanitized;
    }

    /**
     * @brief Merge Education override keys into a resolved CV payload.
     *
     * @param array<string, mixed> $basePayload Default merged CV payload.
     * @param array<string, mixed> $overridePayload Company Education override slice.
     * @return array<string, mixed>
     * @date 2026-06-01
     * @author Stephane H.
     */
