...
class SecurityUiController
{
    ...
    public function loginTotpSubmit(Request $request): Response
    {
        ...
        $csrfToken = (string) $request->request->get('_csrf_token', '');
        if (!$this->csrfTokenManager->isTokenValid(new CsrfToken('authenticate_totp', $csrfToken))) {
            return new Response('', Response::HTTP_FOUND, ['Location' => '/login/totp?error=auth.csrf.invalid']);
        }

        $limiter = $this->loginTotpLimiter->create((string) ($request->getClientIp() ?? 'unknown').'|'.(string) $pendingUserId);
        if (!$limiter->consume()->isAccepted()) {
            return new Response('', Response::HTTP_FOUND, ['Location' => '/login/totp?error=auth.totp.challenge.rate_limited']);
        }
