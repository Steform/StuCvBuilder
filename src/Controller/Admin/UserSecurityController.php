...
class UserSecurityController
{
    ...
    {
        if (!$this->isCsrfTokenValid(self::CSRF_TRUSTED_DEVICE_REVOKE, (string) $request->request->get('_csrf_token', ''))) {
    ...
    {
        if (!$this->isCsrfTokenValid(self::CSRF_TRUSTED_DEVICE_REVOKE_ALL, (string) $request->request->get('_csrf_token', ''))) {
            return $this->csrfDeniedResponse($request, $id);
        }
    ...
    {
        if (!$this->isCsrfTokenValid(self::CSRF_FORCE_PASSWORD_RESET, (string) $request->request->get('_csrf_token', ''))) {
            return $this->csrfDeniedResponse($request, $id);
        }
    ...
    {
        if (!$this->isCsrfTokenValid(self::CSRF_INVALIDATE_SESSIONS, (string) $request->request->get('_csrf_token', ''))) {
            return $this->csrfDeniedResponse($request, $id);
        }
    ...
    /**
     * @brief Validate CSRF token for admin security actions.
     *
    ...
    /**
     * @brief Redirect user detail page after CSRF rejection.
     *
     * @param Request $request Current request.
     * @param int $id Target user identifier.
