...
class UserHardDeleteRestoreService
{
    ...
    public function restoreFromSnapshot(array $snapshotPayload): void
    {
        ...
        $this->entityManager->wrapInTransaction(function () use ($connection, $snapshotPayload): void {
            ...
            $this->upsertRows($connection, 'bug_report', $snapshotPayload['bugReports'] ?? []);
        });
    }

    /**
     * @brief Insert snapshot rows when missing from target table.
     * @param Connection $connection DBAL connection.
     * @param string $tableName Target table name.
     * @param mixed $rows Single row or list of rows.
     * @return void
     * @date 2026-04-28
     * @author Stephane H.
     */
    ...
    /**
     * @brief Normalize DB value for insert binding.
