...
class LocaleConfigurationService
{
    ...
    public function saveConfiguration(array $activeLocales, string $defaultLocale): void
    {
        ...
        if ($normalizedActiveLocales === []) {
            throw new \InvalidArgumentException('dashboard.configuration_language.flash.at_least_one');
        }

        $normalizedDefaultLocale = $this->normalizeLocale($defaultLocale, $normalizedActiveLocales);
        if ($normalizedDefaultLocale === null) {
            throw new \InvalidArgumentException('dashboard.configuration_language.flash.default_must_be_active');
        }
        ...
        if (!is_dir($directory) && !mkdir($directory, 0775, true) && !is_dir($directory)) {
            throw new \RuntimeException(sprintf('Cannot create locale config directory: %s', $directory));
        }
        ...
        if ($written === false) {
            throw new \RuntimeException(sprintf('Cannot write locale config file: %s', $configPath));
        }
    }
