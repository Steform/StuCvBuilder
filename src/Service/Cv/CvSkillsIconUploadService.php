...
/**
 * @brief Store and validate CV skill icon uploads (PNG/WebP conversion, SVG, WebP).
 *
 * @date 2026-05-29
 * @author Stephane H.
 */
final class CvSkillsIconUploadService
{
    ...
    /**
     * @brief Store an uploaded skill icon and return the relative public path.
     *
     * @param UploadedFile $uploadedFile Validated upload.
     * @param string $skillId Skill node id for filename suffix.
     * @return string Relative path under public/.
     * @date 2026-05-29
     * @author Stephane H.
     */
    public function store(UploadedFile $uploadedFile, string $skillId): string
    {
        if (!$uploadedFile->isValid()) {
            throw new \InvalidArgumentException('dashboard.customization_cv.skills.flash_invalid_icon');
        }
        ...
        $safeId = preg_replace('/[^a-z0-9-]/i', '', $skillId) ?? 'skill';
    ...
    /**
     * @brief Delete a previously stored custom skill icon when safe.
     *
    ...
    public function deleteIfNeeded(?string $relativePath): void
    {
        ...
        if (SkillsTreeContract::normalizeIconPath($relativePath) === null) {
            return;
        }
    ...
    {
        $targetDirectory = rtrim($this->projectDir, '/').'/public/'.rtrim(SkillsTreeContract::SKILL_ICON_PATH_PREFIX, '/');
     ...
     * @param string $safeId Skill id fragment.
    ...
    private function storeSvg(UploadedFile $uploadedFile, string $targetDirectory, string $safeId): string
    {
        ...
        $targetFilename = sprintf('skill-%s-%s.svg', substr($safeId, 0, 12), bin2hex(random_bytes(4)));
        $absoluteTarget = $targetDirectory.'/'.$targetFilename;
        if (file_put_contents($absoluteTarget, $contents) === false) {
            throw new \RuntimeException('dashboard.customization_cv.skills.flash_invalid_icon');
        }

        return SkillsTreeContract::SKILL_ICON_PATH_PREFIX.$targetFilename;
    }
    ...
    {
        $targetFilename = sprintf('skill-%s-%s.webp', substr($safeId, 0, 12), bin2hex(random_bytes(4)));
        $absoluteTarget = $targetDirectory.'/'.$targetFilename;
