(function () {
    ...
    var MIN_VERIFY_MS = 2000;
    var MIN_SUCCESS_MS = 250;

    var root = document.querySelector('[data-cv-access-root]');
    var form = document.getElementById('cv-access-form');
    if (!root || !form) {
        return;
    }

    var pointerField = document.getElementById('cv-access-pointer-moves');
    var focusField = document.getElementById('cv-access-focus-events');
    var timeField = document.getElementById('cv-access-time-on-page');
    var webdriverField = document.getElementById('cv-access-webdriver');
    var refreshButton = document.getElementById('cv-access-captcha-refresh');
    var captchaImage = document.querySelector('.cv-access__captcha-image');
    var captchaInput = document.getElementById('captcha-code');
    var phasePanels = root.querySelectorAll('[data-cv-access-phase]');
    ...
    function setPhase(phase) {
        ...
        phasePanels.forEach(function (panel) {
            var isActive = panel.getAttribute('data-cv-access-phase') === phase;
            panel.classList.toggle('d-none', !isActive);
            panel.classList.toggle('cv-access__phase--active', isActive);
        });
    }
    ...
    function isAccessGateRedirect(location) {
        ...
        try {
            ...
            return url.pathname.indexOf('/cv/access') === 0;
        } catch (error) {
            return location.indexOf('/cv/access') >= 0;
        }
    }
    ...
    function redirectToCaptchaPhaseFallback() {
        ...
        if (formatInput && formatInput.value) {
            fallbackUrl.searchParams.set('format', formatInput.value);
        }
    ...
    if (currentPhase === 'captcha') {
        ...
        window.setTimeout(runAutoCheck, MIN_VERIFY_MS);
    }
})();
